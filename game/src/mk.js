;(function () {
  ...
  mk.controllers.Network = function (options) {
    ...
    this._gameName = String(options.gameName || '').trim().toLowerCase();
  ...
  mk.controllers.Network.prototype._initialize = function () {
    ...
    this._transport.on('response', function (response) {
      if (response !== self.Responses.SUCCESS) {
        if (response === self.Responses.GAME_EXISTS) {
          alert('Game already exists or the name is invalid.');
        } else if (response === self.Responses.GAME_NOT_EXISTS) {
          alert('Game not found. Check the game name.');
        } else if (response === self.Responses.GAME_FULL) {
          alert('Game is full.');
        } else {
